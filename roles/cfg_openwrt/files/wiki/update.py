...
"""
This script was derived from the edit_section.py script from the freifunk-berlin repo https://github.com/freifunk-berlin/falter-wiki-bot.
"""
...
INTRO = "<b>Diesen Abschnitt nicht bearbeiten. Änderungen werden automatisch überschrieben!</b><br> Die Konfiguration für diesen Standort wurde mit dem Tool [https://github.com/freifunk-berlin/bbb-configs bbb-configs] erstellt. Der aktuelle Stand der Konfiguration kann dort eingesehen werden. Teile des Wikiartikels werden mit Hilfe von Semantic Values und Templates automatisch erstellt."
...
def find_wiki_page(node: str):
    query = f"{API_URL}?action=ask&query=[[Hat_Knoten%3A%3A{node}]]|%3FModification date|sort%3DModification date|order%3Ddesc&format=json"
...
def find_bbbconfigs_section(pagetitle: str):
    ...
    if bbb_configs_section != -1:
        ...
        raise ValueError(
            "Theres no bbb-configs-section in the given article! See instructions at https://github.com/freifunk-berlin/bbb-configs#automaticly-updating-to-wikifreifunknet"
        )
        exit(1)
...
def edit_section(article_title, section_number, csrf_token, new_text):
    ...
    if data.get("edit").get("nochange") == "":
        print(
            f"SKIPPED: The Wiki article was already up to date. No changes were made. See: {WIKI_URL}/{data.get('edit').get('title')}"
        )
    else:
        print(
            f"UPDATED: The wiki article was successfully updated. Check the results at: {WIKI_URL}/{data.get('edit').get('title')}"
        )
